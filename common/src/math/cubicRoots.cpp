...
// +-------------------------------------------------------------------------
// | COPYRIGHT:
// |    The code in this file was obtained from the following website:
// |        http://van-der-waals.pc.uni-koeln.de/quartic/quartic.html
// |    To the best of my knowledge it is in the public domain.
...
bool cubicRoots(
    ...
    double minx_in, double maxx_in
) {
    double coeff[4] = { A, B, C, D };
    double times[3];
    int    nSolutions;
    cubic(coeff, times, &nSolutions);
    
    uint write = 0;
    for(uint k=0; k<nSolutions; k++) {
        if(times[k] > minx_in && times[k] <= maxx_in) {
            out[write] = times[k];
            write++;
        }
    }
