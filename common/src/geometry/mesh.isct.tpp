...
public:
    ...
    void subdivide(IsctProblem *iprob) {
        ...
        for(IEptr &ie : iedges) {
                ...
                //if(ie->other_tri_key) {
                //    std::cout << "  other tri: "
                //              << ie->other_tri_key->verts[0]->ref
                //              << "; " << ie->other_tri_key->verts[1]->ref
                //              << "; " << ie->other_tri_key->verts[2]->ref
                //              << std::endl;
                //} else {
                //    std::cout << "  other tri: NULL" << std::endl;
                //}
        ...
        if(out.numberofpoints != in.numberofpoints) {
            ...
            std::cout << "dumping out vertex info:" << std::endl;
            for(uint k=0; k<3; k++) {
                std::cout << k << ": id "
                          << overts[k]->concrete->ref << std::endl;
            }
            uint vc = 3;
            for(IVptr iv : iverts) {
                std::cout << "  " << vc++
                          << " (" << iv->glue_marker->edge_tri_type
                          << ";" << iv->glue_marker->split_type
                          << ") ";
            ...
            //std::cout << "dumping out edge info:" << std::endl;
            //for(uint k=0; k<3; k++) {
            //}
            //for(IEptr &ie : iedges) {
            //}
...
template<class VertData, class TriData>
void Mesh<VertData,TriData>::subdivide_helper(
    uint t_piece_ref, uint t_parent_ref
) {
    // ok, we need to figure out whether or not the surface point
    // should be redistributed to this triangle or not...
    SPptr       sp      = tris[t_parent_ref].sp;
    
    Vec3d parent_pos[3];
    for(uint k=0; k<3; k++)
        parent_pos[k] = verts[tris[t_parent_ref].v[k]].pos;
    Vec3d piece_pos[3];
    for(uint k=0; k<3; k++)
        piece_pos[k] = verts[tris[t_parent_ref].v[k]].pos;
    
    // determine which axis to project out, and any sign adjustment
    Vec3d normal = cross(parent_pos[1]-parent_pos[0],
                         parent_pos[2]-parent_pos[0]);
    uint dim = maxDim(abs(normal));
    double sign = (normal[dim] > 0)? 1.0 : -1.0;
    if(len2(normal) == 0) { // handle edge case
        surfps.free(sp);
        tris[t_parent_ref].sp = nullptr;
        return;
    }
    
    // compute the 3d position of the surface point.
    Vec3d sp_pos(0,0,0);
    for(uint k=0; k<3; k++) {
        sp_pos += sp->bary[k] * verts[tris[t_parent_ref].v[k]].pos;
    }
    
    // Then, we recenter the coordinate system on the surface point
    // and compute the offset positions of the three vertices
    // of the fragment triangle.  We project these vectors
    // down into 2 dimensions.
    Vec2f vs[3];
    for(uint k=0; k<3; k++) {
        Vec3d offset = piece_pos[k] - sp_pos;
        vs[k] = proj(dim, offset) * sign;
    }
    
    // now we can easily compute the signed areas
    Vec3d areas;
    double sum_area = 0.0;
    for(uint k=0; k<3; k++) {
        areas[k] = det(vs[(k+1)%3], vs[(k+2)%3]);
        if(areas[k] < 0)
            return; // nope, sp does not lie inside this piece
        sum_area += areas[k];
    }
    areas /= sum_area; // normalize so areas sum to 1
    
    // re-assign sp
    sp->tid = t_piece_ref;
    sp->bary = areas;
    tris[t_piece_ref].sp = sp;
    tris[t_parent_ref].sp = nullptr;
}
...
void Mesh<VertData,TriData>::subdivide_tri(
    ...
    if(tris[t_parent_ref].sp) {
        subdivide_helper(t_piece_ref, t_parent_ref);
    }
