...
{
    // kill any surface point around
    if(mesh->tris[t->ref].sp) {
        mesh->surfps.free(mesh->tris[t->ref].sp);
        mesh->tris[t->ref].sp = nullptr;
    }
    tris.free(t);
}
...
void Mesh<VertData, TriData>::TopoCache::commit()
{
    ...
    // make sure to tend to surface points during this recompaction
    ...
    for(uint read = 0; read < mesh->tris.size(); read++) {
        if(live_tris[read]) {
            ...
            if(mesh->tris[write].sp)
                mesh->tris[write].sp->tid = write; // adjust tid
            write++;
        } else {
            tmap[read] = INVALID_ID;
            // release any surface points here
            if(mesh->tris[read].sp)
                mesh->surfps.free(mesh->tris[read].sp);
                // don't have to set nullptr, cause we're killing
                // the entry in the Mesh::tris array here!
        }
    }
