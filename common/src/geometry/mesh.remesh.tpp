...
inline void Mesh<VertData, TriData>::merge_tris(
    ...
    //std::cout << "triangle merge happening" << std::endl;
    tris[tid_result].sp = (tris[tid0].sp)? tris[tid0].sp : tris[tid1].sp;
    tris[tid_result].sp->tid = tid_result;
    if(tris[tid0].sp && tris[tid1].sp) { // kill t1.sp
        surfps.free(tris[tid1].sp);
    }
    tris[tid0].sp = tris[tid1].sp = nullptr;
...
template<class T> inline
void rotateToFront(int pos, T *v) {
         if(pos == 1)   rotateLeft(v);
    else if(pos == 2)   rotateRight(v);
}
template<class VertData, class TriData>
void Mesh<VertData, TriData>::split_helper(
    uint t0ref, uint t1ref, uint t_orig_ref
) {
    // normalize all of the triangles so that the apex vertex comes first
    // WARNING: we can modify the order so long as we only rotate, never flip
    Tri     *t0         = &(tris[t0ref]);
    Tri     *t1         = &(tris[t1ref]);
    Tri     *t_orig     = &(tris[t_orig_ref]);
    SPptr   sp          = t_orig->sp;
    
    //std::cout << "torig " << t_orig->v[0] << " " << t_orig->v[1] << " "
    //                      << t_orig->v[2] << std::endl;
    //std::cout << "t0 " << t0->v[0] << " " << t0->v[1] << " "
    //                   << t0->v[2] << std::endl;
    //std::cout << "t1 " << t1->v[0] << " " << t1->v[1] << " "
    //                   << t1->v[2] << std::endl;
    //std::cout << "bary " << sp->bary << std::endl;
    // step1: identify the apex and rotate it to the front of each
    // triangle
    int unrotate_orig;
    int unrotate_0, unrotate_1;
    for(uint i=0; i<3; i++) {
        int in0 = -1;
        int in1 = -1;
        for(uint j=0; j<3; j++) {
            if(t_orig->v[i] == t0->v[j])  in0 = j;
            if(t_orig->v[i] == t1->v[j])  in1 = j;
        }
        if(in0 >= 0 && in1 >= 0) {
            rotateToFront(i, t_orig->v);
            rotateToFront(i, sp->bary.v);
            rotateToFront(in0, t0->v);
            rotateToFront(in1, t1->v);
            unrotate_orig = (i==0)? 0 : (i==1)? 2 : 1;
            unrotate_0 = (in0==0)? 0 : (in0==1)? 2 : 1;
            unrotate_1 = (in1==0)? 0 : (in1==1)? 2 : 1;
            break;
        }
    }
    //std::cout << "unrotates " << unrotate_orig << " "
    //          << unrotate_0 << " " << unrotate_1 << std::endl;
    // step 2: flip the identity of t0 and t1 if necessary to ensure that
    //          t0 has t_orig->v[1] in common with t_orig
    if(t0->v[1] != t_orig->v[1]) {
        std::swap(t0, t1);
        std::swap(unrotate_0, unrotate_1);
    }
    
    // now we have a normalized problem where the indexing looks like
    // the following (coords are listed (t_orig, t0, t1)
    //
    //                            *  apex: (0,0,0)
    //                          / | \
    //                        /   |   \
    ...
    //      c1: (1,1,-) * ------- * ------- * c2: (2,-,2)
    //                       mid: (-,2,1)
    Vec3d           c1pos           = verts[t0->v[1]].pos;
    Vec3d           c2pos           = verts[t1->v[2]].pos;
    Vec3d           midpos          = verts[t0->v[2]].pos;
    
    // compute alphas, interpolation parameters between c0 and c1
    double          dist1           = len(midpos-c1pos);
    double          dist2           = len(midpos-c2pos);
    double          alpha1          = dist2 / (dist1 + dist2);
    double          alpha2          = dist1 / (dist1 + dist2);
    if(dist1 + dist2 == 0)
                    alpha1 = alpha2 = 0.5;
    
    // Then our question about the original barycentric coordinates is:
    //      Is sp to the left (c0) or right (c1) of mid?
    // sp is closer to c0 than mid if
    //      bary_orig[1]/(bary_orig[1]+bary_orig[2]) > alpha1/(alpha1+alpha2)
    //      bary_orig[1]*alpha1 + bary_orig[1]*alpha2 >
    //          bary_orig[1]*alpha1 + bary_orig[2]*alpha1
    //      bary_orig[1] * alpha2 > bary_orig[2] * alpha1
    bool in_t0 = (sp->bary[1] * alpha2 > sp->bary[2] * alpha1);
    // handle edge case to prevent divide by zero later
    if(alpha1 == 0) in_t0 = true;
    // We can compute the new barycentric coordinates
    // by rebalancing bary_orig[1] and bary_orig[2].
    // First, rescale bary into alpha space (so that the barys sum to 1)
    //      normbary1 = bary1 / (bary1 + bary2)      (sim. for normbary2)
    // Next, we can compute 1-dimensional alpha values
    //  for the point within the left partition
    //      new_a1 = 1 - normbary2/alpha2
    //      new_a2 = normbary2 / alpha2
    // Then, we compute the new barycentric coordinates by re-adjusting
    // them to fit back in with the third apex coefficient
    //      new_b1 = new_a1 * (bary1 + bary2)   (sim. for new_b1)
    // But, we can collapse all this reasoning to get a nice compact
    // form of the solution
    //      new_b2 = ((bary2 / (bary1+bary2)) / alpha2) * (bary1+bary2)
    //             = bary2 / alpha2
    // and complimentarily
    //      new_b1 = (bary1+bary2) - bary2/alpha2
    //  ********
    // If we tackle the problem next for when the point is on the right
    // we can see that we just need to interchange every occurrence of
    // 1 with 2, which gives us the formulae
    //      new_b1 = bary1 / alpha1
    //      new_b2 = (bary1+bary2) - bary1 / alpha1
    //sp->bary = bary_orig; //(redundant)
    Vec3d   bary_orig   = sp->bary;
    //std::cout << "old bary: " << sp->bary << std::endl;
    if(in_t0) { // point in left
        t0->sp = sp;
        sp->tid = t0ref;
        double quotient = bary_orig[2] / alpha2;
        sp->bary[1] = (bary_orig[1]+bary_orig[2]) - quotient;
        sp->bary[2] = quotient;
        rotateToFront(unrotate_0, sp->bary.v);
    } else { // point in right
        t1->sp = sp;
        sp->tid = t1ref;
        double quotient = bary_orig[1] / alpha1;
        sp->bary[1] = quotient;
        sp->bary[2] = (bary_orig[1]+bary_orig[2]) - quotient;
        rotateToFront(unrotate_1, sp->bary.v);
    }
    //std::cout << "new bary: " << sp->bary << std::endl;
    t_orig->sp = nullptr;
    // unrotate data
    rotateToFront(unrotate_orig, t_orig->v);
    rotateToFront(unrotate_0, t0->v);
    rotateToFront(unrotate_1, t1->v);
}
template<class VertData, class TriData>
inline void Mesh<VertData, TriData>::split_tris(
    uint t0ref, uint t1ref, uint t_orig_ref
) {
    if(tris[t_orig_ref].sp) {
        split_helper(t0ref, t1ref, t_orig_ref);
    }
